...
function loginKlipfolio (callback) {
	...
	klipfolioInstance.goto("https://app.klipfolio.com/")
		...
		.then(() => {
			...
			console.log(`-------------------------------------------`);
			callback(klipfolioInstance);
		})
		.catch(() => console.log(`${osHostname}@:~$ failed to login klipfolio`))
}

function proceedToDataRefresher (klipfolioInstance) {
	const dataSourceIds = JSONDataSourceIds.dataSourceIdList;

	dataSourceLooper(klipfolioInstance, dataSourceIds, 0);
}

function dataSourceLooper (klipfolioInstance, dataSourceIds, index) {
...
function dataSourceRefresher (klipfolioInstance, currentDataSourceId, callback) {
